...
function LinearDiagram(SvgDomElement, Parameters) {
    ...
    var graphic = new SvgGraphic(SvgDomElement);
    var mouseover = false;
    var drawedPoints = [];
    ...
    var groupLegends = new SvgGroup(graphic);

    var groupAxis = new SvgGroup(graphic);
    var hAncillaryLines = new SvgGroup(graphic);
    var hAncillaryLinesLabels = new SvgGroup(graphic);
    var vAncillaryLines = new SvgGroup(graphic);
    var vAncillaryLinesLabels = new SvgGroup(graphic);
    ...
    var groupGraphs = new SvgGroup(graphic);
    var graphLines = new SvgGroup(graphic);
    var graphPoints = new SvgGroup(graphic);
    ...
    var drawLegend = function() {
        ...
        for (var i = options.Data.length - 1, r = 0; i >= 0; i--, r++) {
            ...
            var label = new SvgText(graphic, x, diagramZone.top - 20, options.Data[i].Title);
            label.SetBold(true);
            ...
            var box = new SvgPath(graphic, pathString);
    ...
    var drawAxis = function() {
        //  горизонтальная ось
        var line = new SvgPath(graphic, SvgPath.GeneratePolyline([{x: diagramZone.left, y: diagramZone.bottom }, {x: diagramZone.right, y: diagramZone.bottom }]));
        ...
        for (var i = 1, count = axisHorizontal.length; i < count; i++) {
            ...
            var dottedLine = new SvgPath(graphic, SvgPath.GeneratePolyline([{x: diagramZone.left, y: y}, {x: diagramZone.right, y: y}]));
            ...
            var lineLabel = new SvgText(graphic, diagramZone.left, y, axisHorizontal[i]);
        ...
        for (var i = 0, count = options.Axis.length; i < count; i += verticalAxisOffset.divider) {
            ...
            if ((i > 0) && (i < options.Axis.length - 1)) {
                var verticalLine = new SvgPath(graphic, SvgPath.GeneratePolyline([{x: x, y: diagramZone.top}, {x: x, y: diagramZone.bottom + 10 } ]));
                verticalLine.SetFill(colors.clear);
                verticalLine.SetStroke(1, colors.service);
                vAncillaryLines.Add(verticalLine);
            }

            var lineLabel = new SvgText(graphic, x, diagramZone.bottom, options.Axis[i]);
    ...
    var drawGraph = function() {
        ...
        for (var i = 0, count = options.Data.length; i < count; i++) {
            ...
            var line = new SvgPath(graphic, SvgPath.GeneratePolyline(options.Data[i].Points));
            line.SetFill(colors.clear);
        ...
        for (var i = 0, count = options.Data.length; i < count; i++) {
            for (var p = 0; p < options.Data[i].Points.length; p++ ) {
                ...
                if (point != null) {
                    var circle = new SvgCircle(graphic, point.x, point.y, 3);
    ...
    var infoBox = new SvgInfoBox(graphic, 100, 50, colors.infoboxfill, colors.infoboxtext);
...
function SvgInfoBox(graphic, width, height, backgroundColor, textColor) {
    ...
    var initialize = function() {
        ...
        gradientSvg = new SvgRadialGradient(graphic, [
            { color: "#fff" },
            { color: backgroundColor }
        ]);

        groupSvg = new SvgGroup(graphic);

        containerSvg = new SvgPath(graphic, "M0,0 L0,0");
        gradientSvg.FillTo(containerSvg.Element);

        titleSvg = new SvgText(graphic, 0, 0, "");
        titleSvg.SetFill(textColor);
        titleSvg.SetFontSize("11pt");
        titleSvg.SetBold(true);

        textSvg = new SvgText(graphic, 0, 0, "");
